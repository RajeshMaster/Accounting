...
class SalesplusController extends Controller {
	public static function index(Request $request) {
		...
		if($request->active_select == 2 || $request->active_select == 1) {
			...
			for ($rg=$account_period;$rg>=1;$rg--) {
				...
				$fryr = array();
				$frmnth = array();
				$fil2ndsql=Salesplus::getfilcnt($year_month[$rg][0],$year_month[$rg][1], $rg);
				$cnt_array[$rg] = $fil2ndsql;
			}
		}
		if ($request->active_select == 3) {
			$getmonthtot=Salesplus::fnGetEstimateMonthTotals($date_month,$from_date,$to_date);
			$granttot = 0;
		...
		$sql = Salesplus::fnGetEstimateDetails1s($date_month, $from_date, $to_date, $request);
		if($request->active_select == 2 || $request->active_select == 1) {
			$sql = Salesplus::fnGetEstimateDetailsFlgs($date_month, $from_date, $to_date, $request);
		}
		$sql_sales = Salesplus::fnGetEstimateDetailsales($date_month, $from_date, $to_date, $request);
		if($request->active_select == 2 || $request->active_select == 1) {
			$sql_sales = Salesplus::fnGetEstimateDetailsFlgsales($date_month, $from_date, $to_date, $request);	
		}
		$i=0;
		$temp = "";
		$totalval = 0;
		...
		if($fileCnt >0) {
			...
			if($request->active_select == '3') {
				...
				for ($cnt=0; $cnt<$fileCnt;$cnt++) {
					$filter_month1 = array();
					$customer = Salesplus::selectdetails($get_td[$cnt]['trading_destination_selection']);
					$filpro = 0;
					$result=0;
					$h = 2;
					foreach ($arrval_month1 AS $year => $mvalue) {						
						foreach ($mvalue AS $month => $mmonth) {
							...
							$slemp = Salesplus::fnGetEmply($yearmonth,$get_td[$cnt]['trading_destination_selection']);
							$gettotalval1 = 0;
							foreach ($slemp as $key => $value) {
								$gettotalval1 += Salesplus::fnGetTaxCalculation($value->quot_date,$value->totalval,$value->tax);
							}
						if($fil == 1) {
				...
				for ($cnt=0; $cnt<count($sql_sales);$cnt++) {
					...
					foreach ($arrval_month1 AS $year => $mvalue) {
						foreach ($mvalue AS $month => $mmonth) {
							...
							$yearmonth = $year .'-'. $conmnth;
							$slemptemp = Salesplus::fnGetEmply($yearmonth,$sql_sales[$cnt]->trading_destination_selection);
							$gettotalval123 = 0;
							foreach ($slemptemp as $key => $value) {
								$gettotalval123 += Salesplus::fnGetTaxCalculation($value->quot_date,$value->totalval,$value->tax);
							}
						if($fil == 1) {
		...
		if($request->active_select == '2') {
			...
			for ($cnt=0; $cnt<$fileCnt;$cnt++) {
				...
				for ($l = ($account_period + 2); $l > $account_period; $l--) {
					...
					$indkival = Salesplus::fnGetindikis($startdate,$enddate,$get_td[$cnt]['trading_destination_selection']);
					$getgetindkival = 0;
					foreach ($indkival as $key => $value) {
					$getgetindkival += Salesplus::fnGetTaxCalculation($value->quot_date,$value->totalval,$value->tax);
					}
						if($fil == 1) {
			...
			for ($cnt=0; $cnt<count($sql_sales);$cnt++) {
				...
				for ($l = ($account_period + 2); $l > $account_period; $l--) {
					...
					$indkivalpage = Salesplus::fnGetindikis($startdatepage,$enddatepage,$sql_sales[$cnt]->trading_destination_selection);
					$getgetindkivalpage = 0;
					foreach ($indkivalpage as $key => $value) {
					$getgetindkivalpage += Salesplus::fnGetTaxCalculation($value->quot_date,$value->totalval,$value->tax);
					}
						if($fil == 1) {
				...
				foreach ($dispmnthyr AS $period => $cntvalue) {
					if ($period >= $endperiod) {
						...
						$startdate = $cnt_val_str[0]. "-" . substr("0" . $cnt_val_str[1], -2);
						$enddate = $cnt_val_end[0]. "-" . substr("0" . $cnt_val_end[1], -2);
						$indkival = Salesplus::fnGetindikis($startdate,$enddate,$sql_sales[$cnt]->trading_destination_selection);
						$getgetindkival = 0;
						foreach ($indkival as $key => $value) {
							$getgetindkival += Salesplus::fnGetTaxCalculation($value->quot_date,$value->totalval,$value->tax);
						}
				...
				if ($period >= $endperiod) {
					if ($account_close_mn == 12) {
						...
						for ($i = 1; $i <= $account_close_mn; $i++) {
							...
							$fil1rec = Salesplus::getfil1recs($passyrmn);
							$fil1reckival = 0;
							foreach ($fil1rec as $key => $value) {
								$fil1reckival += Salesplus::fnGetTaxCalculation($value->quot_date,$value->totalval,$value->tax);
							}
							if($fil == 1) {
