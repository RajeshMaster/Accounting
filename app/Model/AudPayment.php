...
use Auth;
use Carbon\Carbon;
class AudPayment extends Model {
		...
		$result=DB::TABLE(DB::raw("(SELECT * FROM(select *,
										...
										from (select invoice.trading_destination_selection AS custid,`invoice`.`user_id` as `invoiceno`, 
													...
													IF(invoice.paid_status=1,
														(SELECT pay.payment_date FROM dev_payment_registration AS pay WHERE LOCATE(CONCAT(',', invid ,','),CONCAT(',',pay.paid_id,',')) LIMIT 1),'') AS invpaymentdate,
