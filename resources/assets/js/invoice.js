...
$(function () {
	$('.invmulticopyprocess').click(function () {
		...
		$("#frminvoicemulticopy").validate({
			...
			submitHandler: function(form) { // for demo
				var checked = $('#workspectable').find(':checked').length;
				if (checked==0) {
			...
			if ($('#quot_date').val() == '' || $('#payment_date').val() == '') {
				$('[name*="quot_date"]').each(function () {
					$(this).rules('add', {
						required: true,
						messages: {
...
function newexceldownload(invid,mainmenu) {
		var confirm_create="Do you Want to Create Invoice";
		if(confirm(confirm_create)) {
			$('#frminvoicespec').attr('action', 'newexceldownloadprocess?mainmenu='+mainmenu+'&time='+datetime);
			$("#frminvoicespec").submit();
		}
}
...
function allinvoiceexceldownload(mainmenu) {
	var invChecked = new Array();
	if($('.checkboxid:checkbox:checked').length > 0){
		$('.checkboxid:checkbox:checked').each(function() {
	      invChecked[invChecked.length] = this.value;            
	    });
		$('#hdn_invoice_arr').val(invChecked);

		var confirm_create="Do you Want to Create Invoice";
		if(confirm(confirm_create)) {
			$('#frmallinvoiceexceldownload').attr('action', 'allinvoiceexceldownloadprocess?mainmenu='+mainmenu+'&time='+datetime);
			$("#frmallinvoiceexceldownload").submit();
		}
	} else {
		alert("Please Select Invoice ID");return;
	}
...
function fnWorkspecificEmpidDisable(name) { 
	var filedval = $('#'+name).val();
	var id = name.replace("work_specific", "");
	var kananame = "empKanaNames"+id;
	var empids = "emp_ID"+id;
	if (filedval == "") {
		$('#'+kananame).text('');
...
function fnCheckboxVal1(id,loopvalue) {
	// alert(id);
	// alert(loopvalue);
		//$('#work_specific_21_1').attr('disabled','disabled');
