...
<style type="text/css">
	...
	@if(isset($detedit))
		...
		{{ Form::model($detedit, array('name'=>'salaryaddedit', 'id'=>'salaryaddedit', 'files'=>true,'type'=>'file', 'method' => 'POST','class'=>'form-horizontal','url' => 'Salary/addeditprocess?time='.date('YmdHis') ) ) }}

	@else

		{{ Form::open(array('name'=>'salaryaddedit', 'id'=>'salaryaddedit','type'=>'file', 'url' => 'Salary/addeditprocess?mainmenu='.$request->mainmenu.'&time='.date('YmdHis'),'files'=>true,
	...
	<div class="row hline pm0">
		...
		<div class="col-xs-12">
			...
			<h2 class="pull-left pl5 mt10">
					...
					{{ trans('messages.lbl_salary') }}<span class="">・</span>@if ($request->editflg=="2")<span style="color:red">{{ trans('messages.lbl_edit') }}</span>@elseif ($request->editflg=="3")<span style="color:green">{{ trans('messages.lbl_copy') }}</span>@else<span style="color:green">{{ trans('messages.lbl_register') }}</span>@endif
	...
	<fieldset>
		...
		<div id="salaryhdn" class="col-xs-12 mt5" <?php if(isset($detedit[0]->bankId) && $detedit[0]->bankId == "999" || $request->bank == "999"){ ?> style="margin-bottom: 10px !important;" <?php } ?>>
			...
			<div class="col-xs-9">
				...
				@else
					...
					{{ Form::text('txt_salary',(isset($register[0]->salary)) ? $register[0]->salary : '',array(
		...
		<div class="col-xs-12 mt5 mb10" id="divcharge" <?php if($chargedef){ ?> style="display:none;" <?php } ?>>
			...
			<div class="col-xs-9">
				...
				@if(isset($detedit))
					...
					{{ Form::text('charge',$detedit[0]->charge,array(
				...
				@else
					...
					{{ Form::text('charge',(isset($register[0]->charge)) ? $register[0]->charge : '',array(
	...
	{{ Form::open(array('name'=>'salaryaddeditcancel', 'id'=>'salaryaddeditcancel', 'url' => 'Salary/addeditprocess?mainmenu='.$request->mainmenu.'&time='.date('YmdHis'),'files'=>true,'method' => 'POST')) }}
