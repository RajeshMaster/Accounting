...
<article id="expenses" class="DEC_flex_wrapper " data-category="expenses expenses_sub_5">
	...
	{{ Form::open(array('name'=>'salarymultiadd', 'id'=>'salarymultiadd', 'url' => 'Salary/multiaddeditprocess?mainmenu='.$request->mainmenu.'&time='.date('YmdHis'),'files'=>true,
	...
	<div class="row hline pm0">
		...
		<div class="col-xs-12">
			...
			<h2 class="pull-left pl5 mt10">
					...
					{{ trans('messages.lbl_salary') }}<span class="">・</span><span style="color:green">{{ trans('messages.lbl_register') }}</span>
	...
	<fieldset class="ml10 mr10">
		...
		<div class="box100per  mt15" align="center">
					...
					{{ Form::selectMonthbynumber('salarymonth', '', 
			...
			<table class="box98per CMN_tblfixed tablealternate ml10 mb10" id = "workspectable">
				...
				<thead class="CMN_tbltheadcolor">
					...
					<tr class="">
						...
						<th class="vam">{{ trans('messages.lbl_process') }}</th>
				...
				@for ($i=0; $i<$fileCnt;$i++)
					...
					<tr>
						...
						<td>
									...
									{{ Form::text('salary'.$i,(isset($estimate[0]->$remarksloop)) ? $estimate[0]->$remarksloop : '',array('id'=>'salary'.$i,
														...
														'class'=>'box99per form-control pl5 mt3 ime_mode_disable numonly')) }}
						...
						<td style="text-align: center;">
							...
							@if($get_multisalaryId[$i]['previous'] != 0)

								<a title="Get Previous Salary" href="javascript:fngetpreviousdetails('{{ $i }}','{{  $request->selMonth }}','{{ $request->selYear }}','{{ $request->mainmenu }}');" style="padding:3px 4px;" class="btn btn-info active box100">

									<span class=""></span>

										{{ trans('messages.lbl_pgetprevsal') }}
		...
		<div class="form-group">
			...
			<div align="center" class="mt5">
				...
				@if($request->multiflg == 1)
					...
					<a onclick="javascript:gotoindexmultipage('2','{{ $request->mainmenu }}');" class="btn btn-danger box120 white"><i class="fa fa-times" aria-hidden="true"></i> {{trans('messages.lbl_cancel')}}

					</a>

				@else

					<button type="submit" class="btn edit btn-warning box100 multiaddeditprocess">

						<i class="fa fa-edit" aria-hidden="true"></i> {{ trans('messages.lbl_update') }}

					</button>

					<a onclick="javascript:gotoindexmultipage('1','{{ $request->mainmenu }}');" class="btn btn-danger box120 white"><i class="fa fa-times" aria-hidden="true"></i> {{trans('messages.lbl_cancel')}}
...
{{ Form::open(array('name'=>'salarymultiaddeditcancel', 'id'=>'salarymultiaddeditcancel', 'url' => 'Salary/multiaddeditprocess?mainmenu='.$request->mainmenu.'&time='.date('YmdHis'),'files'=>true,'method' => 'POST')) }}
