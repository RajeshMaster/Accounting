...
<style type="text/css">
	...
	{{ Form::open(array('name'=>'salaryview', 'id'=>'salaryview', 'url' => 'Salary/index?mainmenu='.$request->mainmenu.'&time='.date('YmdHis'),'files'=>true,
		  'method' => 'POST')) }}
	...
	<div class="row hline pm0">
		<div class="col-xs-12">
			...
			<h2 class="pull-left pl5 mt10">
					{{ trans('messages.lbl_salary') }}<span>・</span><span class="colbl">{{ trans('messages.lbl_view') }}</span>
			</h2>
		</div>
	</div>
	...
	<fieldset>
		...
		<div class="col-xs-12 mt5">
			<div class="col-xs-3 text-right clr_blue">
				<label>{{ trans('messages.lbl_charge') }}</label>
			</div>
			<div>
				@if(!empty($singleview[0]->charge))
					@if(isset($singleview[0]->charge))
						{{ $singleview[0]->charge }}
					@elseif($singleview[0]->charge == 0)
						NIL
					@else
						NIL
					@endif
