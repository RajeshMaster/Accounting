...
<article id="sales" class="DEC_flex_wrapper " data-category="sales sales_sub_9">
	{{ Form::open(array('name'=>'frmsalesindex', 
						'id'=>'frmsalesindex', 
						'url' => 'Salesplus/index?mainmenu='.$request->mainmenu.'&time='.date('YmdHis'),
						'files'=>true,
						'method' => 'POST')) }}
...
<!-- Start Heading -->
	<div class="row hline">
		<div class="col-xs-12">
			...
			<h2 class="pull-left pl5 mt15">{{ trans('messages.lbl_sales_dep') }}</h2>
		</div>
	</div>
<!-- End Heading -->
				...
				<thead class="CMN_tbltheadcolor">
			   		<tr class="tableheader tac"> 
				  		...
				  		<?php 
							...
							foreach ($cnt_array AS $period => $cntvalue) {
								if ($period >= $endperiod) { ?> 
									<th class="exedet_table_text " id="elastic_cus<?php echo $period;?>" width=<?php echo $thwth;?> align='center'>{{ intval($period) }} 
