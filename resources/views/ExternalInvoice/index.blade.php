	...
	<div class="col-xs-12 pm0 pull-left">
		...
		<div class="col-xs-12 pm0 pull-left">
			<div class="box55per pm0 CMN_display_block pull-left">
				...
				<span>|</span>
				<a class="btn btn-link {{ $disabledcreating }}" 
					href="javascript:filter('2');">{{ trans('messages.lbl_creating') }} 
				</a>
				<span>|</span>
				<a class="btn btn-link {{ $disabledapproved }}" 
					href="javascript:filter('3');">{{ trans('messages.lbl_approved') }}
				</a>
				<span>|</span>
				<a class="btn btn-link {{ $disabledunused }}" 
					href="javascript:filter('4');">{{ trans('messages.lbl_unused') }}
				</a>
				<span>|</span>
				<a class="btn btn-link {{ $disabledsend }}" 
					href="javascript:filter('5');">{{ trans('messages.lbl_sent') }} 
				</a>
			</div>
			<div class="pm0 pr12">
				<div class="form-group pm0 pull-right moveleft nodropdownsymbol" 
					id="moveleft">
							...
							<td class="tal pr10 vat pt5">
								...
								<div>
									<span class="estStatusDIV_New_1">
										@if($data->classification == 0 && $data->delFlg == 0)
											<span class="blue">
												{{ trans('messages.lbl_creating') }}
											</span>
										@elseif($data->classification == 1 && $data->delFlg == 0)
											<span class="orange">
												{{ trans('messages.lbl_approved') }}
											</span>
										@elseif($data->classification==2 && $data->delFlg == 0)
											<span class="green">
							...
							<td>
								...
								<div class="ml5 mb2 smallBlue">
									...
									<div class="CMN_display_block ml3">
										...
										<a href="javascript:invoicenewpdf('{{ $data->id }}','{{ $data->pdfFlg }}','{{ $data->id}}pdfimg');"  class="anchorstyle ml3">{{ trans('messages.lbl_invoice') }}</a>&nbsp;
										<span class="ml3">|</span>
									</div>
									<div class="CMN_display_block ml3 dropdown">
										<a href="#" style="text-decoration: none !important;" class="anchorstyle">
											{{ trans('messages.lbl_Others') }}</a>
										<div class="CMN_display_block" >
		...
		<div class="scrollbar">
				...
				<li class="theme-option ml6">
					<div class="box100per mt5" 
						...
						{!! Form::text('singlesearch', $request->singlesearch,
								...
								'style'=>'height:30px;','id'=>'singlesearch')) !!}
						{{ Form::button('<i class="fa fa-search" aria-hidden="true"></i>', array('class'=>'ml5 mt2 pull-left search box15per btn btn-info btn-sm', 
							'type'=>'submit',
							'name' => 'advsearch',
			...
			<ul id="demo" @if ($request->searchmethod == 2) class="collapse in ml5 pull-left" @else class="collapse ml5 pull-left" @endif>
				<li class="theme-option" onKeyPress="return checkSubmitmulti(event)">
					...
					<div class="mt5">
						<span class="pt3" style="font-family: arial, verdana;">
							{{ trans('messages.lbl_daterange') }}</span>
						<div class="mt5 box100per">
							...
							!!}
							<label class="ml1 mt8 fa fa-calendar fa-lg pull-left" 
								for="msearchstdate" aria-hidden="true"></label>
							{!! Form::text('msearcheddate', $request->msearcheddate,
								array('',
									...
									'id'=>'msearcheddate')) 
							!!}
							<label class="ml1 mt8 fa fa-calendar fa-lg" 
								for="msearcheddate" aria-hidden="true"></label>
						</div>
					</div>
					<div class="mt5 mb6">
