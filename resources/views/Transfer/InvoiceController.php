...
class InvoiceController extends Controller {
    function index(Request $request) {
        ...
        $othersArray = array('0' => trans('messages.lbl_creating'),
                             '1' => trans('messages.lbl_invapproved'),
...
public static function addeditprocess(Request $request){
    ...
    if ($request->regflg==1 || $request->regflg==2) {
        ...
        if($insert) {
            ...
            $code = Invoice::fnGenerateInvoiceID($request);
            $update = Invoice::fnUpdateInvoicedetails($request,$code,$totalinvvalue,$getbranchid[0]->id);
            $getqdate=array();
            $retedit=1;
...
public static function addeditinv(Request $request) {
    ...
    if(empty($invoicedata[0]->totalval)) {
        $invoicedata[0]->totalval=0;
        $grandtotal = 0;
        $dispval = 0;
    }
       ...
       $totalval = 0;
		...
		Tel:06-6305-1251,Fax:06-6305-1250";   */    
		   ...
		   // $pdf->MultiCell(65, 4, mb_convert_encoding($display, 'SJIS', 'UTF-8'), 0,'L', 0);   
		    ...
		    $pdf->SetXY(14.5, 218.1);         
		    $pdf->Cell(25, 6.3, iconv('UTF-8', 'SJIS', "口座番号  "), 'L', 0, 'C', true); 
		    $pdf->SetXY(39.5, 218.1);         
		    $pdf->Cell(5, 6.1, "", 0, 0, 'C');         
		    $pdf->SetXY(44.5, 218.1);         
		    ...
		    for ($i = 1; $i <= 19; $i++) {
		        ...
		        if($i==19){
		            ...
		            $pdf->SetXY(19.5, 96.9+$y);         
		            $pdf->Cell(74.8, 6.0301, "", 'T', 0, 'L', true);
		            $pdf->SetXY(19.5, 96.9+$y);         
		            $pdf->drawTextBox(mb_convert_encoding($in_query[0]->$work_specificarr, 'SJIS', 'UTF-8'), 74.8, 6.0301, 'L', 'B', 0);
		            if(!empty($in_query[0]->$quantityarr)) {
		                // CHANGED BY BABU
		   ...
		   if(count($arrval) != 0) {
		        ...
		        for($i = 0; $i<count($arrval); $i++) {
		            ...
		            $no=($rccnt-$exvalue).")";
		            $pdf->SetXY(22.5 ,247.5+$y );
		            $pdf->Write(6, iconv('UTF-8', 'SJIS', $no ));
		            $pdf->SetFont( 'MS-Mincho' ,'B',10);
		            $pdf->SetXY(26.5 ,246+$y );
		            $dispStr = $arrval[$i];
  ...
  // send mail process start 
    ...
    function exceldownloadprocess(Request $request) {
            ...
            $grandtotal = "";
            if (!empty($getinvoicedetails[0]->totalval) && !empty($getinvoicedetails[0]->invoice_id)) {
            ...
            foreach ($getinvoicedetails as $key=> $value){
				...
				for ($i=1; $i <=19 ; $i++) { 
					if(($i%2) == 0) {
			            $objTpl->getActiveSheet()->getStyle('B' . ($cellval + $i))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
			            $objTpl->getActiveSheet()->getStyle('B' . ($cellval + $i))->getFill()->getStartColor()->setRGB('DCDCDC');
			            $objTpl->getActiveSheet()->getStyle('R' . ($cellval + $i))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
			            $objTpl->getActiveSheet()->getStyle('R' . ($cellval + $i))->getFill()->getStartColor()->setRGB('DCDCDC');
			            $objTpl->getActiveSheet()->getStyle('C' . ($cellval + $i))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
			            $objTpl->getActiveSheet()->getStyle('C' . ($cellval + $i))->getFill()->getStartColor()->setRGB('DCDCDC');
			            $objTpl->getActiveSheet()->getStyle('U' . ($cellval + $i))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
			            $objTpl->getActiveSheet()->getStyle('U' . ($cellval + $i))->getFill()->getStartColor()->setRGB('DCDCDC');
			            $objTpl->getActiveSheet()->getStyle('X' . ($cellval + $i))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
			            $objTpl->getActiveSheet()->getStyle('X' . ($cellval + $i))->getFill()->getStartColor()->setRGB('DCDCDC');
			            $objTpl->getActiveSheet()->getStyle('AC' . ($cellval + $i))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
			            $objTpl->getActiveSheet()->getStyle('AC' . ($cellval + $i))->getFill()->getStartColor()->setRGB('DCDCDC');
			            //client excel
			            $objTpl->getActiveSheet()->getStyle('B' . ($clientcellval + $i))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
			            $objTpl->getActiveSheet()->getStyle('B' . ($clientcellval + $i))->getFill()->getStartColor()->setRGB('DCDCDC');
			            $objTpl->getActiveSheet()->getStyle('R' . ($clientcellval + $i))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
			            $objTpl->getActiveSheet()->getStyle('R' . ($clientcellval + $i))->getFill()->getStartColor()->setRGB('DCDCDC');
			            $objTpl->getActiveSheet()->getStyle('C' . ($clientcellval + $i))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
			            $objTpl->getActiveSheet()->getStyle('C' . ($clientcellval + $i))->getFill()->getStartColor()->setRGB('DCDCDC');
			            $objTpl->getActiveSheet()->getStyle('U' . ($clientcellval + $i))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
			            $objTpl->getActiveSheet()->getStyle('U' . ($clientcellval + $i))->getFill()->getStartColor()->setRGB('DCDCDC');
			            $objTpl->getActiveSheet()->getStyle('X' . ($clientcellval + $i))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
			            $objTpl->getActiveSheet()->getStyle('X' . ($clientcellval + $i))->getFill()->getStartColor()->setRGB('DCDCDC');
			            $objTpl->getActiveSheet()->getStyle('AC' . ($clientcellval + $i))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
			            $objTpl->getActiveSheet()->getStyle('AC' . ($clientcellval + $i))->getFill()->getStartColor()->setRGB('DCDCDC');
        			}
   				 }
    ...
    function invoiceexceldownloadprocess(Request $request) {
        ...
        foreach ($TotEstquery as $key => $value) {
            ...
            $objTpl->getActiveSheet()->setCellValue('H'.$x, $dispval1);
